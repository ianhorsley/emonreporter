"""Test script to explorer getting passworded xml file and parsing"""
...
def get_headers(url):
    h = requests.head(url, allow_redirects=False, auth=(username,password))
    print h
    return h.headers
...
def has_changed(header, destfile):
    """Has the file changed compared to existing file if exists."""
...
if is_downloadable(headers) and has_changed(headers, destfile):
    print("downloading updated file")
    response = requests.get(url, allow_redirects=False, auth=(username,password))
...
for stat in stats[0:5]:
    ...
    for key, group in itertools.groupby(targetlist, key=lambda target:target['time'].date()):
        ...
        for id, target in enumerate(daytargets):
            if id in gapstoremove:
